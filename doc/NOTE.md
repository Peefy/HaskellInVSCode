...
但是-有时候我们确实希望能够做这样的事情！如果使用Haskell唯一可以做的事情是编写函数，然后我们可以在ghci提示符下对其进行评估，那么从理论上讲这很有趣，但实际上没有用。

事实上，这是可以做到的这些事情哈斯克尔，但看起来比大多数其他语言有很大不同。
...
r :: Recipe Cake
你有什么？一块蛋糕？不，你有一些说明如何做蛋糕，只是对一些书面的纸张。



不仅您实际上没有蛋糕，仅拥有食谱对其他任何事物都没有影响。只需将食谱握在手中，就不会导致烤箱变热或面粉洒在地板或任何类似物品上。要真正制作蛋糕，必须遵循食谱（导致面粉撒出，配料混合，烤箱变热等）。



同样，类型IO a值只是产生类型值的“配方” a（可能在此过程中会产生一些影响）。像任何其他值一样，它可以作为参数传递，作为函数的输出返回，存储在数据结构中，或者（如我们很快将看到的）与其他IO值组合成更复杂的配方。
...
如果您希望遵循自己的食谱，那么最好将其作为大食谱（main）的一部分，交给大厨。当然，main可以任意复杂，并且通常由许多较小的IO计算组成。

因此，让我们编写第一个实际的可执行的Haskell程序！我们可以使用功能

putStrLn :: String -> IO ()
给定一个String，它返回一个IO计算结果（执行后）String在屏幕上打印出来。因此，我们只需将其放入一个名为Hello.hs：

main = putStrLn "Hello, Haskell!"
然后runhaskell Hello.hs在命令行提示符下键入，导致我们的消息被打印到屏幕上！我们还可以ghc --make Hello.hs用来生成一个称为Hello（或Hello.exe在Windows上）的可执行版本。

没有String“内部”IO String
许多新的Haskell用户最终在某个时候问诸如“我有一个IO String，如何将它变成一个String？”或“如何String摆脱这个IO String”之类的问题？根据上述直觉，应该清楚这些是无意义的问题：type的值IO String是对某种计算的描述，一个配方，用于生成一个String。有没有String“内部”的IO String，不是有一个蛋糕一个蛋糕食谱“内部”了。要生产String（或美味的蛋糕），需要实际执行计算（或配方）。唯一的方法是通过将其（可能是更大IO值的一部分）提供给Haskell运行时系统main。

结合 IO
到现在应该很清楚了，我们需要一种将 IO计算合并为更大的方法。

结合两个IO计算的最简单方法是使用(>>)运算符（发音为“ and then”），其类型为

(>>) :: IO a -> IO b -> IO b
这简单地创建了一个IO计算，该计算包括按顺序运行两个输入计算。注意，第一次计算的结果将被丢弃；我们只关心它的作用。例如：

main = putStrLn "Hello" >> putStrLn "world!"
这对于“执行此操作；做这个; 这样做”，结果并不重要。但是，通常这是不够的。如果我们不想丢弃第一次计算的结果怎么办？

解决这种情况的第一个尝试可能是拥有某种类型的东西IO a -> IO b -> IO (a,b)。但是，这也不足够。原因是我们希望第二个计算能够依赖于第一个计算的结果。例如，假设我们要从用户那里读取一个整数，然后打印出比他们输入的整数多一个的整数。在这种情况下，第二个计算（在屏幕上打印一些数字）将根据第一个计算的结果而有所不同。

而是有一个运算符(>>=)（发音为“ bind”），其类型为

(>>=) :: IO a -> (a -> IO b) -> IO b
首先这可能很难缠住你的头！(>>=)需要将产生类型的值的运算a，和一个功能，其到达计算基于类型的该中间值的第二计算a。结果(>>=)是（对a的描述）计算，该计算将执行第一个计算，使用其结果来确定下一步要执行的操作，然后执行该操作。

例如，我们可以编写一个程序来读取用户的号码并打印出其后继者。请注意，我们使用的readLn :: Read a => IO a是从用户读取输入并将其转换为的任何类型的计算的计算Read。

main :: IO ()
main = putStrLn "Please enter a number: " >> (readLn >>= (\n -> putStrLn (show (n+1))))
当然，这看起来很难看，但是有更好的编写方法，我们将在以后讨论。

记录语法
该材料未在讲座中涵盖，但作为完成作业8的额外资源提供。

假设我们有一个数据类型，例如

data D = C T1 T2 T3
我们还可以使用记录语法声明此数据类型，如下所示：

data D = C { field1 :: T1, field2 :: T2, field3 :: T3 }
我们不仅为存储在构造函数中的每个字段指定类型，而且为其指定名称C。新版本的D可以与旧版本完全相同的方式使用（特别是我们仍然可以对Das 类型的值进行构造和模式匹配C v1 v2 v3）。但是，我们获得了一些额外的好处。

每个字段名称自动是一个投影函数，该函数从type的值中获取该字段的值D。例如，field2是一个函数类型

field2 :: D -> T2
以前，我们将不得不field2通过编写代码来实现自己

field2 (C _ f _) = f
如果我们具有许多字段的数据类型，那么这将消除很多样板！
...
用...正确类型的表达式填充。请注意，我们可以按任何顺序指定字段。

假设我们有一个价值d :: D。我们可以使用如下语法进行修改 d

d { field3 = ... }
当然，通过“修改”我们并不是要真正地变异d，而是要构造一个新的类型值，该类型值与新字段D相同，d只是用field3给定值替换了字段。

最后，我们可以像这样对类型的值进行模式匹配D：
