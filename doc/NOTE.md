...
Haskell非常擅长抽象：参数多态性，高阶函数和类型类之类的函数都有助于对抗重复。
...
## Haskell组合函数
通过组合许多简单函数来构建更复杂的函数是一种很好的Haskell风格。
```
...
实际上，模式的完整语法仍然包含更多函数，但是其余的函数暂时将带到了很远的地方。
...
当然，改变的是要对列表的每个元素执行的操作。可以将此操作指定为type 的函数Int -> Int。在这里，开始看到能够将函数作为输入传递给其他函数有多么巨大的用处！
...
```
类型Maybe a的值要么包含类型的值a（包装在Just构造函数中），要么包含Nothing（表示某种故障或错误）。该Data.Maybe模块具有处理Maybe值的函数。
...
哪些函数可以具有这种类型？类型表示给定类型的事物列表a，函数必须产生一些类型的值a。例如，Prelude函数head具有这种类型。

…但是如果head输入一个空列表怎么办？让来看看源代码，为head...

它崩溃了！由于它必须适用于所有类型，因此它可能无能为力。没有办法凭空组成任意类型的元素。

head这就是所谓的局部函数：某些输入head会崩溃。具有某些输入将使它们无限递归的函数也称为局部函数。在所有可能的输入上定义明确的函数称为总函数。

Haskell的最佳实践是尽可能避免局部函数。实际上，在任何编程语言中，避免使用局部函数都是一种好习惯，但是在大多数情况下，这是令人讨厌的。Haskell倾向于使其变得非常容易和明智。

head是个错误！它不应该在中Prelude。其他部分Prelude，几乎从来不应该使用的函数包括tail，init，last，和(!!)。从这一点开始，在作业中使用以下函数之一将失去样式得分！
...
事实证明，它foo是真正的`(.)`，代表函数的组成。也就是说，如果f和g是函数，则f . g先执行g然后再执行的函数f。
...
还记得多参数函数的类型看起来很奇怪，就像它们中有“多余”箭头吗？例如考虑函数
```hs
f :: Int -> Int -> Int
f x y = 2 * x + y
```
...
f' x y = 2*x + y
```
特别要注意的是，右侧的函数箭头`W -> X -> Y -> Z`等效于`W -> (X -> (Y -> Z))`。始终可以在类型中最右边的顶级箭头周围添加或删除括号。
...
如果要实际表示两个参数的函数，则可以使用一个**元组**的单个参数。即函数
```hs
f'' :: (Int,Int) -> Int
f'' (x,y) = 2*x + y
```
...
```
uncurry特别是当有一对想要对它应用函数时，该函数很有用。例如：
```
Prelude> uncurry (+) (2,3)
5
```

Haskell中的函数被咖喱化的事实使得部分应用特别容易。部分应用的想法是，可以采用多个参数的函数，并将其仅应用于其某些参数，然后得出其余参数的函数。但是，正如刚刚看到的，在Haskell 中没有多重参数的函数！每个函数都可以“部分地”应用到其第一个（也是唯一一个）参数，从而产生其余参数的函数。
...
让将一个示例中刚刚学到的东西放在一起，该示例还显示了“全餐”编程风格的强大函数。考虑foobar定义如下的函数：
```hs
...
编码的这种风格中，定义一个函数不参照它的参数，在一定意义上说的函数什么的，而不是什么它做 -is被称为“自由点”的风格。从上面的示例可以看出，它可能非常漂亮。甚至有人甚至说应该始终使用无点样式。但是太远了，它会变得非常混乱。lambdabot在#haskellIRC信道有一个命令@pl用于使函数集成到相当的自由点表达式; 这是一个例子：
```hs
...
```
这三个函数有什么共同点，有什么不同？与往常一样，该想法将是借助定义高阶函数的能力来抽象出变化的部分。
```hs
fold :: b -> (a -> b -> b) -> [a] -> b
fold z f []     = z
...
当然，fold已经在标准Prelude中以名称提供了foldr。参数的foldr顺序略有不同，但函数完全相同。以下是一些Prelude函数，它们的定义如下foldr：
```hs
length :: [a] -> Int
sum :: Num a => [a] -> a
product :: Num a => [a] -> a
...
a -> a -> a
```
请记住，a是一个类型变量，可以代表任何类型。这种类型的函数有哪些？
...
a -> a -> a
```
作为承诺，与这种类型的函数将工作不管是什么类型的调用者进行选择。
...
考虑以下每种多态类型。对于每种类型，确定该类型的函数可能具有的行为。
```hs
a -> a
a -> b
a -> b -> a
...
* Ord用于其元素可以完全排序的类型，即可以比较任何两个元素以查看哪个元素小于另一个元素。它提供了类似的比较操作(<)和(<=)，也是compare函数。
...
```
如果根据实现其他函数toList，它们也会受到Listable约束。例如：
```hs
...
但是，在Haskell中，可以定义短路运算符而没有任何特殊情况。事实上，(&&)和(||)只是打算旧库的函数！例如，这(&&)是定义方式：
```hs
(&&) :: Bool -> Bool -> Bool
True  && x = x
False && _ = False
...
前面的章节中已经了解了如何为列表定义折叠函数，但是也可以将其推广到其他数据类型！
...
* 函数可能没有任何外部影响。例如，某个函数可能无法在屏幕上打印任何内容。函数只能计算其输出。
* 函数可能不取决于外部因素。例如，它们可能无法从键盘，文件系统或网络读取。函数可能仅取决于其输入，换句话说，函数每次都应为相同的输入提供相同的输出。
...
因此，让编写第一个实际的可执行的Haskell程序！可以使用函数
```hs
putStrLn :: String -> IO ()
```
...
(>>=) :: IO a -> (a -> IO b) -> IO b
```
首先这可能很难缠住的头！(>>=)需要将产生类型的值的运算a，和一个函数，其到达计算基于类型的该中间值的第二计算a。结果(>>=)是（对a的描述）计算，该计算将执行第一个计算，使用其结果来确定下一步要执行的操作，然后执行该操作。
...
也就是说，如果具有`Name`和`String`，则可以应用`Employee`构造函数来构建`Employee`对象。

但是，假设没有`a Name`和`a String`；实际拥有的是一个`Maybe Name`和一个`Maybe String`。也许它们来自解析充满错误的某些文件，或者来自某些字段可能留为空白的形式，或者来自某种形式。不一定可以构造一个Employee。但是当然可以构造一个Maybe Employee。也就是说，希望采用的`(Name -> String -> Employee)`函数并将其转换为`(Maybe Name -> Maybe String -> Maybe Employee)`函数。
...
可以想象它是如何工作的：如果名称或字符串是`Nothing`，那就为Nothing；如果两者都为Just，则Employee使用Employee构造函数（封装在中Just）得出一个构建体。但是，让继续前进...

考虑一下：现在有`a Name`和`a `而不是`String a [Name]`和`a [String]`。
...
可以想象两种不同的工作方式：可以将对应的`Names`和`Strings`匹配为`Employees`；或者可以通过所有可能的方式将`Names`和`Strings`配对。

还是这样：有一个`(e -> Name)`和`(e -> String)`用于某种类型e。例如，也许e是一些巨大的数据结构，并且有一些函数告诉如何从中提取`a Name`和`a String`。可以将其制成一个`(e -> Employee)`，即Employee从相同结构中提取的方法吗？
...
既然已经看到了这种模式的用处，让概括一下。想要的函数类型实际上看起来像这样：

```hs
(a -> b -> c) -> (f a -> f b -> f c)
```

嗯，这看起来很熟悉……与`fmap`的类型非常相似！

```hs
fmap :: (a -> b) -> (f a -> f b)
```

唯一的区别是额外的论点。可以调用所需的函数`fmap2`，因为它需要两个参数的函数。也许可以写fmap2的方面fmap，所以只需要Functor对约束f：
```hs
fmap2 :: Functor f => (a -> b -> c) -> (f a -> f b -> f c)
fmap2 h fa fb = undefined
```

尽力而为，但是Functor并不能给足够的执行力`fmap2`。
...
注意，也可以写成has 的类型`a -> (b -> c)`。因此，有一个带有的函数a，并且有一个类型的值f a…唯一能做的就是使用fmap将该函数移到上f，从而得到一个类型的结果：
...
现在有了某种类型f (b -> c)和某种类型f b……，这就是被困住的地方！fmap不再有帮助。它为提供了一种将函数应用于Functor上下文中的值的方法，但是现在需要的是将自身在Functor上下文中的函数应用于上下文中的值Functor。
...
`(<*>)`运算符（通常发音为“AP”，简称“申请”）封装“上下文应用程序”的正是这个道理。还要注意的是，Applicative类需要它的实例是实例Functor为好，这样就可以随时使用fmap与实例Applicative。最后，请注意，Applicative还有另一种方法，pure它允许将类型的值注入a到容器中。现在，有趣的是，这fmap0将是pure：
...
现在`(<*>)`可以实现`fmap2`，它在标准库中实际上称为`liftA2`：
...
实际上，这种模式非常普遍，因此Control.Applicative定义为`(<$>)`的同义词fmap，
...
（注意，`(<$>)`和`(<*>)`的优先级和结合性实际上以这样的方式，上述的所有的括号是不必要的定义）。

好漂亮！与从`fmap`到`liftA2`（需要从Functor到泛化Applicative）的跳转不同，从liftA2到liftA3（从那里到liftA4…）跳转不需要任何额外的函数— Applicative足够。

其实，当拥有所有的参数，像这样平时也懒得调用`liftA2`，`liftA3`等，但只使用`f <$> x <*> y <*> z <*> ...`直接模式。（不过liftA2，朋友确实会派上用场进行部分应用。）

但是呢pure？pure是想要的一些函数应用到论据一些仿函数的背景情况f，但是一个或多个自变量是不是在f-those参数是“纯粹的”，可以这么说。在申请之前，可以先将pure其提升为f第一个。像这样：
...
映射函数f在容器x应，得到相同的结果第一注入函数放入容器中，然后将其施加到x与(<*>)。

### 适用实例

让尝试以Applicative开头的一些实例Maybe。pure通过将值注入Just包装器来工作；`(<*>)`是可能失败的函数应用程序。结果是Nothing函数或其参数是否为。
...
每个Applicative也是一个-那么Functor可以fmap根据pure和实施`(<*>)`。

```hs
fmap g x = pure g <*> x
```

但是，不难想象对于某种类型的make Functor和Applicativeinstance使得这种相等性不成立。由于这将是一个相当可疑的情况，因此将平等性规定为一条法律，这是一种正式方法，表明给定类型的Functor和Applicative实例必须“很好地配合”。

Applicativefor列表的实例如何？实际上有两种可能的实例：一种将元素列表的函数列表和参数列表匹配（即，将它们“压缩”在一起），另一种实例以所有可能的方式组合函数和参数。

首先，让编写执行所有可能组合的实例。（出于下周将变得清楚的原因，这是默认实例。）从这个角度来看，列表代表不确定性：也就是说，类型的值`[a]`可以被认为是具有多种可能性的单个值。然后`(<*>)`对应于不确定性函数应用程序，即，将不确定性函数应用于不确定性参数。
...
也许这个特定的例子没有多大意义，但是不难想象想以各种可能的方式组合事物的情况。例如，可以像这样进行非确定性算法：
...
接下来，编写一个进行元素组合的实例。首先，必须回答一个重要的问题：应该如何处理不同长度的列表？一些想法表明，最明智的做法是将较长的列表截短为较短的列表，丢弃多余的元素。当然，还有其他可能的答案：例如，可以通过复制最后一个元素来扩展较短的列表（但是，当其中一个列表为空时，该怎么办？）；或使用“中性”元素扩展较短的列表（但随后将需要为的实例Monoid，或应用程序的额外“默认”参数）。

该决定反过来决定了pure必须遵守法律的方式，因为必须遵守法律

```hs
pure f <*> xs === f <$> xs
```

请注意，右侧是长度与列表相同的列表xs，通过应用f到中的每个元素形成xs。可以使左侧结果相同的唯一方法是…… pure创建的无限多个副本f，因为事先不知道xs会持续多久。

使用newtype包装器实现实例，以将其与其他列表实例区分开。标准的Prelude函数zipWith也很方便。
...
做一个最后的示例实例`(->) e`。这被称为阅读器或适用于环境的应用程序，因为它允许从“环境”中“读取” e。实现实例并不难，只需要使用鼻子并遵循以下类型：
...
Functor是一个漂亮的工具，但相对简单。乍看之下，似乎Applicative并没有增加太多Functor，但事实证明，这只是一个很小的增加而产生的巨大影响。Applicative应该被称为“计算模型”，而Functor事实并非如此。

在处理诸如Applicative和时Monad，请务必记住涉及多个层次的抽象。粗略地说，抽象是一种隐藏了较低级别细节的东西，它提供了一个“高层”接口，可以使用该接口而无需考虑较低级别—尽管较低级别的细节通常会“泄漏”在某些情况下。抽象层的想法很普遍。考虑一下用户程序-OS-内核-集成电路-门-硅或HTTP-TCP-IP-以太网，或编程语言-字节码-汇编-机器代码。如所见，Haskell为提供了许多不错的工具，可用于在Haskell程序本身中构造多层抽象，也就是说，可以向上动态扩展“编程语言”层堆栈。这是一个强大的函数，但可能导致混乱。必须学会明确地能够在多个层次上思考，并在各个层次之间进行切换。

对于Applicative和Monad特别，只有两个层面去关注。第一个是实施各种水平Applicative和Monad实例，即 “原始的Haskell”电平。当实现的Applicative实例时，在以前的家庭作业中获得了有关此级别的经验Parser。

一旦有了Applicative类型为的实例Parser，关键是可以使用接口 “向上移动层”并使用Parsers 进行Applicative编程，而无需考虑如何实际实现实例Parser及其Applicative实例的细节。在上周的家庭作业中对此有了一点经验，并且本周将获得更多的经验。与实际实现实例相比，在此级别进行编程具有完全不同的感觉。让看一些例子。

### 适用的API

拥有统一接口的好处之一Applicative是可以编写适用于任何类型的通用工具和控制结构的实例Applicative。作为第一个示例，让尝试编写

```hs
pair :: Applicative f => f a -> f b -> f (a,b)
```

pair接受两个值并将它们配对，但是都在some的上下文中Applicative f。第一次尝试，可以使用一个函数来配对并使用`(<$>)`and 将其“提升”到参数上`(<*>)`：

```hs
pair fa fb = (\x y -> (x,y)) <$> fa <*> fb
```

尽管可以简化一点，但这是可行的。首先，请注意，Haskell允许使用特殊语法`(,)`来表示对构造函数，因此可以编写

```hs
pair fa fb = (,) <$> fa <*> fb
```

但是实际上，之前已经看过这种模式-这是使liftA2Applicative踏上整条路的模式。所以可以进一步简化为

```hs
pair fa fb = liftA2 (,) fa fb
```

但是现在无需显式写出函数参数，因此可以达到最终的简化版本：

```hs
pair = liftA2 (,)
```

现在，此函数有什么作用？当然，这取决于f所选的特定对象。让考虑一些特定的例子：

f = Maybe：结果是Nothing两个参数中的任意一个为 如果两者都是Just结果就是Just他们的配对。
f = []：pair计算两个列表的笛卡尔乘积。
f = ZipList：pair与标准zip函数相同。
f = IO：依次pair执行两个IO操作，并返回一对结果。
f = Parser：pair依次运行两个解析器（解析器使用输入的连续部分），并成对返回其结果。如果任何一个解析器失败，则整个事情都会失败。
可以实现以下函数吗？考虑一下f用上述每种类型替换时每个函数的函数。
...
之前已经看到了Applicative该类如何使能够惯用地处理在某种“特殊上下文”中进行的计算-例如，考虑到可能出现的故障，带有的Maybe多个可能的输出[]，咨询了某种在环境中使用((->) e)，或在作业中使用“组合器”方法构造解析器。

但是，到目前为止，只看到了具有固定结构的计算，例如将数据构造函数应用于固定的参数集。如果事先不知道计算的结构怎么办–也就是说，希望能够根据一些中间结果来决定要做什么？

举个例子，Parser从家庭作业中回忆类型，并假设已经实现了它Functor和Applicative实例：
...
假设有函数`parseInt :: Parser Int`和`parseChar :: Parser Char`。的Applicative实例自动处理可能失败（如果解析的任何组件失效，解析整个Foo将失败），并通过的未消耗部分螺纹String输入依次每个组件。

但是，假设试图解析一个包含数字序列的文件，如下所示：
```hs
4 78 19 3 44 3 1 7 5 2 3 2
```
要注意的是，文件中的第一个数字告诉下一组“数字”的长度；组之后的下一个数字是下一组的长度，依此类推。因此，以上示例可以分为以下几类：

* `78 19 3 44`   -- first group
* `1 7 5`        -- second group
* `3 2`          -- third group

这是一个示例，但是实际上，许多“真实世界”文件格式遵循类似的原理，即读取某种标头，然后告诉随后几个块的长度，或者在哪里可以找到内容和文件等。
...
不幸的是，仅使用Applicative接口是不可能的。问题在于，Applicative无法根据先前的结果来决定下一步该怎么做：在看到结果之前，必须预先决定要运行的解析操作。
...
这看起来应该很熟悉！以前在的上下文中已经看到了这些方法IO，但是实际上它们并不是特定IO于所有的。IO事实证明，对Monadic的接口只是有用的。

return也看起来很熟悉，因为它具有与相同的类型pure。实际上，每个Monad也都应该是Applicative带有的pure = return。两者都有的原因是已经存在了一段时间之后Applicative才发明的。 Monad

`(>>)`只是`(>>=)`的专用版本（Monad如果某些实例想要提供更有效的实现，则将其包含在类中，但通常默认实现就可以了）。因此要了解它，首先需要了解(>>=)。

实际上有第四种方法称为fail，但是将其放在Monad类中是一个错误，并且永远不要使用它，因此不会告诉（如果有兴趣，可以在Typeclassopedia中阅读它）。

`(>>=)`（发音为“绑定”）是所有动作的所在！让仔细考虑一下它的类型：

* `(>>=) :: m a -> (a -> m b) -> m b`
* (>>=)有两个参数。第一个是type的值m a。（顺便说一句，此类值有时称为单子值或计算。还建议将它们称为Mobits。不能称其为“ monads”，因为那是一种错误：类型构造函数m是monad 。）在任何情况下，想法都是类型的Mobit m a表示导致类型的值（或多个值，或者没有值）的计算a，并且还可能具有某种“效果”：

`c1 :: Maybe a`是可能会失败但a会成功的计算。

`c2 :: [a]`是导致（多个）as 的计算。

`c3 :: Parser a`是隐式消耗a的一部分String并且（可能）产生a的计算a。

`c4 :: IO a`是一种可能具有某些I/O效果的计算，然后生成一个a。

`(>>=)`是类型`(a -> m b)`的函数。也就是说，它是一个函数，它将根据第一个计算的结果选择要运行的下一个计算。这恰恰体现了Monad封装计算的能力，该计算可以根据先前的计算结果选择下一步做什么。

因此，`(>>=)`真正要做的就是将两个移动点组合在一起以产生一个较大的移动点，然后先运行第一个，然后再运行另一个，返回第二个的结果。最重要的转折是，要根据第一个输出的输出来决定运行哪个移动位。
...
return当然是Just。如果的第一个参数`(>>=)`是Nothing，则整个计算将失败；否则，如果为`Just x`，则应用第二个参数x来决定下一步要做什么。

顺便提一句，k第二个参数通常使用字母，(>>=)因为k代表“继续”。希望在开玩笑。
...
Monad类的一个好处是，仅使用`return`，`(>>=)`可以构建许多不错的通用组合器，以便使用monad进行编程。
