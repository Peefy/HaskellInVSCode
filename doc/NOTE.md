...
```
uncurry特别是当您有一对想要对它应用功能时，该功能很有用。例如：
```
Prelude> uncurry (+) (2,3)
5
```
...
编码的这种风格中，定义一个函数不参照它的参数，在一定意义上说的功能什么的，而不是什么它做 -is被称为“自由点”的风格。从上面的示例可以看出，它可能非常漂亮。甚至有人甚至说您应该始终使用无点样式。但是太远了，它会变得非常混乱。lambdabot在#haskellIRC信道有一个命令@pl用于使功能集成到相当的自由点表达式; 这是一个例子：
```hs
...
（如果fold从这个角度考虑，您也许可以弄清楚如何归纳fold为列表以外的数据类型……）
